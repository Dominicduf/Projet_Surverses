...
def line_chart(df, mode, name):
    ...
    if mode =='Durée de déversement':
        ...
        fig.update_traces(hovertemplate=hover_template.linechart_dure_hover_template())
    elif mode =='Fréquence de déversements':
        fig = px.scatter(df, x='Année', y='Fréquence', color='Contexte du débordement', title=titre, color_discrete_sequence=px.colors.qualitative.Alphabet, color_discrete_map=dict_color)
        fig.update_traces(hovertemplate=hover_template.linechart_freq_hover_template())

    fig.layout.paper_bgcolor ="rgb(209, 222, 224)"
    fig.update_layout(showlegend=False,hoverlabel=dict(bgcolor="white", font_size=14))
    return fig
